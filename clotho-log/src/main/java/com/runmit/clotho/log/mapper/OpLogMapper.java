...
public interface OpLogMapper {
	 ...
	 List<OpLog> getOpLogs(@Param("start")int start,@Param("limit")int limit,@Param("opType")OpType opType,@Param("opMod")String opMod,@Param("systemId")int systemId);
	 
	 class GetOpLogs{
		 public String getOpLogs(Map<String, Object> para){
			 StringBuilder sb = new StringBuilder("select * from OpLog where id!=0");
			 if(para.get("opType")!=null){
			 ...
			 if((Integer)para.get("systemId")!=0){
